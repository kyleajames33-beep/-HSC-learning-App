...
// Content Access Tracking Hook for Module 5 Biology Pathway
export const useModule5Progress = (userId = 'demo-user') => {
  const [pathwayProgress, setPathwayProgress] = useState(null);
  const [loading, setLoading] = useState(true);

  // Initialize user progress structure
  const initializeProgress = useCallback(() => {
    const savedProgress = localStorage.getItem(`module5-progress-${userId}`);
    ...
    return {
      ...
      unlockedDotPoints: ['IQ1.1', 'IQ1.2', 'IQ1.3', 'IQ2.1', 'IQ2.2', 'IQ3.1', 'IQ3.2', 'IQ3.3', 'IQ4.1', 'IQ4.2', 'IQ4.3', 'IQ5.1', 'IQ5.2'], // FORCE ALL UNLOCKED
  ...
  const saveProgress = useCallback((updatedProgress) => {
    localStorage.setItem(`module5-progress-${userId}`, JSON.stringify(updatedProgress));
  ...
  const trackContentAccess = useCallback((dotPointId, contentType, contentId) => {
    ...
    console.log(` Content accessed: ${dotPointId} - ${contentType} (All accessed: ${allContentAccessed})`);
  ...
  const submitQuizAttempt = useCallback((dotPointId, quizType, score, totalQuestions, timeSpent) => {
    ...
    const timestamp = new Date().toISOString();
    const updatedProgress = { ...pathwayProgress };
    const passingScore = BIOLOGY_MODULE_5_PATHWAY.requiredPassPercentage; // 65%
    ...
    if (dotPointCompleted && !updatedProgress.completedDotPoints.includes(dotPointId)) {
      ...
      if (nextDotPoint && !updatedProgress.unlockedDotPoints.includes(nextDotPoint)) {
        ...
        console.log(` Unlocked next dot point: ${nextDotPoint}`);
      }
    }
    ...
    console.log(` Quiz completed: ${dotPointId} - ${quizType} - ${score}% (${passed ? 'PASSED' : 'FAILED'})`);
  ...
  const calculateOverallProgress = (progress) => {
    const totalDotPoints = 13; // 3+2+3+3+2
  ...
  // Check if quiz is available (always available for unlocked dotpoints)
  const isQuizAccessible = useCallback((dotPointId, quizType) => {
    // Always accessible if dotpoint is unlocked (no content requirements)
    if (!isContentAccessible(dotPointId)) return false;
    
    const quizProgress = pathwayProgress?.quizProgress?.[dotPointId];
    
    // Allow access even if no quiz progress exists yet
    if (!quizProgress) return true;

    if (quizType === 'quickQuiz') {
      return !quizProgress.quickQuizPassed; // Can attempt if not already passed
    } else if (quizType === 'longResponse') {
      return !quizProgress.longResponsePassed; // Can attempt if not already passed
    }
  ...
  const resetProgress = useCallback(() => {
    localStorage.removeItem(`module5-progress-${userId}`);
